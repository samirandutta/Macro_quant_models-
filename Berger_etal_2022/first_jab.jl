...
# # ------------------------------------------------------------------------------------
# pgf_wage_bill_share = pgf_plot(raw"C. Wage Payment Share: $s_{ij}^{wn}$", raw"", :s)
# pgfsave("wage_bill_share_plot.tex", pgf_wage_bill_share)

# # 4. Labor Elasticity Plot 
# # ------------------------------------------------------------------------------------------------------------
# pgf_labor_elasticity = pgf_plot(raw"D. Labor Supply Elasticity: $\epsilon_{ij}(s_{ij}^{wn})$", raw"", :eps)
...
#     {
#         title = raw"E. Markdown: $\mu_{ij} = \frac{\epsilon_{ij}(s_{ij}^{wn})}{\epsilon_{ij}(s_{ij}^{wn}) + 1}$",
#         xlabel = raw"Productivity percentile, $z_{ij}$",
#         ylabel = raw"",
#         xtick = x_positions,
#         xticklabels = percentile_labels,
